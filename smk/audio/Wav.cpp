...
namespace smk {
	...
	bool Wav::open(AudioFormat const& format, std::string const& filename) {
		close();
		...
		if (m->file)
		{
			...
			header->SampleRate = uint32_t(format.sample_rate);
			header->ByteRate = uint32_t(format.sample_rate * format.channels * sample_bytes);
			header->BlockAlign = format.channels * sample_bytes;
	...
	bool Wav::load(std::string const &filename, std::vector<float> &data, AudioFormat &format) {
		...
		if (ok) {
			...
			format.sample_rate = header.SampleRate;
