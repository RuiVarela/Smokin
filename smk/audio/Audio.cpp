...
namespace smk {
	...
	uint64_t audioMicroseconds(uint64_t samples, AudioFormat const& format) { 
		return (samples * Microseconds) / (format.sample_rate * format.channels); 
	}

	uint64_t audioMilliseconds(uint64_t samples, AudioFormat const& format) { 
		return (samples * Milliseconds) / (format.sample_rate * format.channels); 
	}

	uint64_t samplesFromMicroseconds(uint64_t microseconds, AudioFormat const& format) { 
		return ((microseconds * format.sample_rate) / Microseconds) * format.channels; 

	}

	uint64_t samplesFromMilliseconds(uint64_t milliseconds, AudioFormat const& format) {
		 return ((milliseconds * format.sample_rate) / Milliseconds) * format.channels;
	}
	...
	std::string timecode(uint64_t milliseconds) {
        ...
        char buffer[15];
		snprintf(buffer, sizeof(buffer),"%02d:%02d:%02d.%03d", int(hours), int(minutes), int(seconds), int(ms));
	...
	static std::string noteName(int note, bool octave, std::string const names[12]) {
		if (octave) {
			...
			return names[note % 12] + to_str8(note_octave);
		}
